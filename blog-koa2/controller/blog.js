...
const getList = async (author, keyword) => { // 获取博客列表 这个就是假装根据了传入的参数返回对应的数据！（格式至少是正确的）
    ...
    return await exec(sql) // 传入函数得到一个promise，返回给路由
}

const getDetail = async (id) => { // 获取博客详情
...
const newBlog = async (blogData = {}) => { // 新建博客
...
const updateBlog = async (id, blogData = {}) => { // 更新博客
